...
$get_transaksi = $db->query("SELECT ro.created_at,ro.nama_obat,SUM(ro.jumlah_retur) as jumlah FROM `parent_retur_obat` pr INNER JOIN rincian_retur_obat ro ON(pr.id_parent_retur=ro.id_parent_retur) WHERE pr.id_parent_retur='".$id_parent_retur."' GROUP BY ro.nama_obat ORDER BY ro.created_at LIMIT $limit_awal,$limit_akhir");
  ...
  <?php echo $body; ?>
    ...
    <table id="head_kop" border="0" width="100%">
      ...
      <tr>
        ...
        <td width="35%">: <?php echo $head['ruangan']." / ".$head['petugas_retur']; ?></td.
      </tr>
    </table>
    <table border="1" width="100%">
      ...
      <?php
        for ($i=0; $i <=10 ; $i++) {
          ...
          $col1 = $i;
          $col2 = $i+11;
          $nama_col_1 = isset($trans[$col1]['nama_obat']) ? $trans[$col1]['nama_obat'] : '';
          $nama_col_2 = isset($trans[$col2]['nama_obat']) ? $trans[$col2]['nama_obat'] : '';
          $vol_col_1 = isset($trans[$col1]['jumlah']) ? $trans[$col1]['jumlah'] : '';
          $vol_col_2 = isset($trans[$col2]['jumlah']) ? $trans[$col2]['jumlah'] : '';
          echo '<tr>
            <td align="center" width="3%">'.$no.'</td>
            <td width="40%">'.$nama_col_1.'</td>
            <td align="center">'.$vol_col_1.'</td>
            <td align="center" width="3%">'.$no2.'</td>
            <td width="40%">'.$nama_col_2.'</td>
      ...
      ?>
		...
		<table width="100%">
			<tr>
				...
				<td style="font-size:14px;text-align:center;">Total Harga : <?php echo "Rp.".number_format($head['total_retur'],0,'.','.'); ?></td>
				<td style="text-align:center;">
