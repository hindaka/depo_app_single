...
$id_transaksi_obat = isset($_GET['t']) ? $_GET['t'] : '';
$tipe_depo = $tipes[2];
$conf = json_decode(file_get_contents("config/env_depo.json"), true);
$id_depo = $conf[$tipe_depo]["id_depo"];
$header = $db->query("SELECT * FROM rincian_obat_pasien ro INNER JOIN registerpasien rp ON(ro.id_pasien=rp.id_pasien) WHERE ro.id_rincian_obat='" . $id_rincian_obat . "'");
$head = $header->fetch(PDO::FETCH_ASSOC);
$get_ranap_igd = $db->query("SELECT rd.*,g.nama as namaobat,g.sumber FROM rincian_detail_obat rd INNER JOIN gobat g ON(rd.id_obat=g.id_obat) WHERE rd.id_trans_obat='" . $id_transaksi_obat . "'");
          ...
          <div class="col-md-12">
            <div class="box" style="border-top:3px solid #605ca8">
              <div class="box-header">
                ...
                <h3 class="box-title">Data Rincian Transaksi ( <?php echo $head['nomedrek'] . " | " . $head['nama'] . " | " . $head['dpjp']; ?>)</h3>
