...
$nama=$data4["nama"];
//get list obat
$list_obat = $db->query("SELECT ws.id_warehouse_stok,ws.id_warehouse,ws.id_obat,ws.stok,g.nama,g.sumber FROM warehouse_stok ws INNER JOIN warehouse w ON(w.id_warehouse=ws.id_warehouse) INNER JOIN gobat g ON(g.id_obat=ws.id_obat) WHERE w.id_warehouse='".$id_depo."' AND ws.stok>0");
...
<html>
  ...
  <body class="<?php echo $skin_depo; ?>">
	...
	<?php } else if (isset($_GET['status']) && ($_GET['status'] == "3")) { ?><div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><center><h4><i class="icon fa fa-ban"></i>Peringatan!</h4>Stok obat tidak mencukupi / volume obat tidak boleh kosong!</center></div>
	    <?php } ?>
	    <!-- end pesan -->
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li>Transaksi</li><li class="active">Obat Keluar</li>
          </ol>
        </section>
					...
					<div class="form-group">
                      ...
                      <!-- <input type="text" class="form-control" id="namaobat" name="namaobat" placeholder="Nama Obat" autocomplete="off" required> -->
                        ...
                        <option value="">---Pilih Obat---</option>
												<?php
													foreach ($obat as $o) {
														echo "<option value='".$o['id_warehouse_stok']."'>".$o['nama']." - ".$o['stok']." - ".$o['sumber']."</option>";
													}
												 ?>
                      </select>
                    </div>
										...
										<?php
											...
											<tfoot>
												<tr class="info">
													...
													<td colspan="2"><b>Rp.<?php echo number_format($subtot,0,'.','.'); ?></b></td>
												</tr>
												<tr class="warning">
													<td colspan="4" class="text-right"><b>Pembulatan</b></td>
													<td colspan="2"><b>Rp.(<?php echo number_format(pembulatan($subtot)-$subtot,0,'.','.'); ?>)</b></td>
												</tr>
												<tr class="success">
													<td colspan="4" class="text-right"><b>Total yang harus dibayarkan</b></td>
													<td colspan="2"><b>Rp.<?php echo number_format(pembulatan($subtot),0,'.','.'); ?></b></td>
												</tr>
											</tfoot>
											</table>
                    </div>
