...
$volume_return = isset($detail['volume']) ? $detail['volume'] : 0;
$id_obat = isset($detail['id_obat']) ? $detail['id_obat'] : 0;
//get data rincian
...
if ($total_trans > 0) {
	...
	if ($total_wo > 0) {
		...
		$ref_id = isset($kartu['ref']) ? $kartu['ref'] : '';
		// update stok
		...
		$get_stok = $db->query("SELECT volume_kartu_akhir FROM kartu_stok_ruangan WHERE id_kartu_ruangan='" . $ref_id . "'");
		$stok = $get_stok->fetch(PDO::FETCH_ASSOC);
		$stok_akhir = isset($stok['volume_kartu_akhir']) ? $stok['volume_kartu_akhir'] : 0;
		$final_stok = $stok_akhir + $volume_return;
		$up_stok = $db->query("UPDATE kartu_stok_ruangan SET volume_kartu_akhir='$final_stok' WHERE id_kartu_ruangan='" . $ref_id . "'");
