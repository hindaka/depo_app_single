	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<?php if (isset($_GET['status']) && ($_GET['status'] == "1")) { ?><div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
					<center>
						<h4><i class="icon fa fa-check"></i>Berhasil</h4>Data SO berhasil ditambahkan
					</center>
				</div>
			<?php } ?>
			<!-- end pesan -->
	...
	<script type="text/javascript">
		...
		$(function() {
			master_so = $('#example2').DataTable({
				"processing": true,
				"serverSide": true,
				"ajax": "ajax_data/stok_opnam_depo_sync.php?bulan=<?php echo $bulan; ?>&tahun=<?php echo $tahun; ?>",
				"pageLength": 10,
				"columns": [{
						"data": "tanggal_so"
					},
					...
					},
					{
						...
						"render": function(data, type, row) {
							var btn;
							if (data.sync == 'y') {
								btn = '<button class="btn btn-block btn-sm btn-success disabled"><i class="fa fa-pencil"></i> SYNC SO</button>';
								btn += '<a href="stok_opnam_depo_cetak_hasil.php?ctx=' + data.id_so + '" target="_blank" class="btn btn-block btn-sm bg-purple"><i class="fa fa-print"></i> Cetak Form SO</a>';
							} else {
								btn = '<a href="stok_opnam_depo_sync_acc.php?sync=' + data.id_so + '" class="btn btn-block btn-sm btn-primary"><i class="fa fa-gears"></i> SYNC SO</a>';
								btn += '<a href="stok_opnam_depo_cetak_hasil.php?ctx=' + data.id_so + '" target="_blank" class="btn btn-block btn-sm bg-purple"><i class="fa fa-print"></i> Cetak Form SO</a>';
							}
							return btn;
						}
