...
if ($jenis == 'generik') {
	$kartu_list = $db->query("SELECT * FROM kartu_stok_ruangan WHERE id_warehouse='" . $id_depo . "' AND volume_kartu_akhir>0 AND id_obat='" . $id_obat . "' AND jenis='" . $jenis . "' AND pabrikan='" . $merk_pabrikan . "' ORDER BY expired ASC");
	$kartu_app = $kartu_list->fetchAll(PDO::FETCH_ASSOC);
} else if ($jenis == 'non generik') {
	$kartu_list = $db->query("SELECT * FROM kartu_stok_ruangan WHERE id_warehouse='" . $id_depo . "' AND volume_kartu_akhir>0 AND id_obat='" . $id_obat . "' AND jenis='" . $jenis . "' AND merk='" . $merk_pabrikan . "' ORDER BY expired ASC");
	$kartu_app = $kartu_list->fetchAll(PDO::FETCH_ASSOC);
} else if ($jenis == 'bmhp') {
	$kartu_list = $db->query("SELECT * FROM kartu_stok_ruangan WHERE id_warehouse='" . $id_depo . "' AND volume_kartu_akhir>0 AND id_obat='" . $id_obat . "' AND jenis='" . $jenis . "' (merk='" . $merk_pabrikan . "' OR pabrikan='" . $merk_pabrikan . "') ORDER BY expired ASC");
	$kartu_app = $kartu_list->fetchAll(PDO::FETCH_ASSOC);
} else {
	$kartu_list = $db->query("SELECT * FROM kartu_stok_ruangan WHERE id_warehouse='" . $id_depo . "' AND volume_kartu_akhir>0 AND id_obat='" . $id_obat . "' ORDER BY expired ASC");
	$kartu_app = $kartu_list->fetchAll(PDO::FETCH_ASSOC);
}
...
if ($total_data > 0) {
	while ($turn_off == false) {
		...
		$ins_kartu->execute();
		$id_kartu_ruangan_last = $db->lastInsertId();
		...
		$ins_rincian->bindParam(":ruang", $ruang, PDO::PARAM_STR);
	...
	(:tanggal_keluar,:id_kartu_ruangan,:id_obatkeluar,:id_warehouse_stok,:volume,:harga_beli,:harga_satuan,:total_harga,:jenis_rawat,:id_resep,:id_detail_rincian)");
		...
		$ins_out->bindParam(":id_kartu_ruangan", $id_kartu_ruangan_last, PDO::PARAM_INT);
	...
	echo "<script language=\"JavaScript\">window.location = \"obat_ranap_detail.php?id=" . $id_rincian . "&t=" . $id_transaksi . "&r=" . $setRuang . "&status=1\"</script>";
} else {
	//obat tidak ditemukan
	header("location:obat_ranap_detail.php?id=" . $id_rincian . "&t=" . $id_transaksi . "&r=" . $setRuang . "&status=3");
	exit;
}
